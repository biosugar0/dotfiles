...
#alias claude='cage --preset claude-code -- claude --dangerously-skip-permissions --append-system-prompt "$(uvx --from git+https://github.com/oraios/serena serena print-system-prompt)"'
alias claude='claude --dangerously-skip-permissions --mcp-config $XDG_CONFIG_HOME/claude/mcp.json --append-system-prompt "$(uvx --from git+https://github.com/oraios/serena serena print-system-prompt)"'

#alias codex='cage --preset codex -- codex --dangerously-bypass-approvals-and-sandbox'
...
# git worktree (git gtr)
alias gwn='git gtr new'
alias gwl='git gtr list'
gwr() {
  local selected name
  selected=$(git gtr list --porcelain 2>/dev/null | fzf --delimiter='\t' \
    --with-nth=2 \
    --preview 'git -C {1} log --oneline -10' \
    --preview-window=right:50%)
  [[ -n "$selected" ]] && name=$(echo "$selected" | cut -f2) && git gtr rm "$name"
}
alias gwra='git gtr rm --all'
gwa() {
  local selected name
  selected=$(git gtr list --porcelain 2>/dev/null | fzf --delimiter='\t' \
    --with-nth=2 \
    --preview 'git -C {1} log --oneline -10' \
    --preview-window=right:50%)
  [[ -n "$selected" ]] && name=$(echo "$selected" | cut -f2) && git gtr ai "$name" -- "$@"
}
gwe() {
  local selected name
  selected=$(git gtr list --porcelain 2>/dev/null | fzf --delimiter='\t' \
    ...
    --preview 'git -C {1} log --oneline -10' \
    --preview-window=right:50%)
  [[ -n "$selected" ]] && name=$(echo "$selected" | cut -f2) && git gtr editor "$name"
}
alias gwrun='git gtr run'
gw() {
  local selected
  selected=$(git gtr list --porcelain 2>/dev/null | fzf --delimiter='\t' \
    --with-nth=2 \
