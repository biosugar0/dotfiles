return {
  ...
  },
  {
    ...
    config = function()
      ...
      -- diffの出力を取得する関数
      local function gitdiff(select, source)
        ...
        if result_staged and result_staged ~= '' then
          ...
          select_buffer.lines = parse_diff(result_staged) -- 構造化したdiffをlinesに格納
        ...
        select_buffer.filetype = 'diff'
        select_buffer.lines = parse_diff(result_default) -- 構造化したdiffをlinesに格納
        return select_buffer
      end
...
- その他の提案: "suggestion:"
          ...
          selection = function(source)
            ...
            local selection_table = gitdiff(select, source)
