...
local utils = {
  ...
  is_cache_valid = function(cache_entry)
    return cache_entry.timestamp > 0 and (os.time() - cache_entry.timestamp) < CACHE_TTL
  end,
  ...
  create_popup_window = function(response)
    ...
    -- calculate_popup_sizeのロジックを直接実装
    ...
    keys = {
      ...
      -- Diagnostic Fixing (replacing CopilotChatFixDiagnostic)
      {
        ...
        function()
          ...
          local current_file = vim.fn.expand('%:p')
          local prompt = prompts.system .. '\n\n' .. diag_context .. string.format(prompts.diagnostic_fix, current_file)
          execute_prompt(prompt, true)
        end,
        desc = '🔧 診断エラー修正',
      },
