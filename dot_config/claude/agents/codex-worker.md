...
guard は wrapper 非経由の `codex exec` と、許可ブロック内の exfil 系トークン(`curl`/`wget`/`nc`/`ssh`/`scp`)を拒否する。
...
worktree 作成後の codex 実行は必ず `--cd "$WT"` を使う。`git wt` と `--cd "$PWD"` を同じ Bash ブロックに入れない。
