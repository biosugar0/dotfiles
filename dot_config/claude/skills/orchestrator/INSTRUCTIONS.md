...
### 検証コマンド
- `npm test -- --grep "feature"` → 0 failures
...
**Contract が有効な場面（限定的に使用）:**
- ステップの完了判定が曖昧で、過去に「できた」と主張して壊れていた経験がある場面
- evaluator skill との連携時（Contract が evaluator の入力になる）
- 外部 API やインフラ変更など、rollback コストが高い作業

**Contract が不要な場面（デフォルト）:**
- 単純な1ファイル変更
- 機械的な置換作業
- 調査・探索のみのステップ
- スコープが探索中に動くことが予想されるステップ
...
- 各ステップ完了後、結果をレビュー
- Contract がある場合: 各項目を検証コマンドで確認
  - FAIL 項目がある場合: 修正サブタスクを追加して再実行
  - 全 PASS → 次ステップへ
  - evaluator skill が有効な場合: Contract + 成果物を evaluator に渡す
- 残りのステップがまだ適切か検証
- 発見に基づいて次のステップを調整
- サブタスクの追加・削除・変更を行う
