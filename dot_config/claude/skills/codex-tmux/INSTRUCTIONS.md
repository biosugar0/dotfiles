...
tmux list-panes -a -F '#{pane_id}' | grep -qx "$TMUX_PANE" \
...
STATE_FILE="${XDG_STATE_HOME:-$HOME/.local/state}/codex-tmux/state.env"
...
# post-verify: split が PARENT_WIN に着地したか必ず確認
# pane_id ターゲット指定 (-t "$TMUX_PANE") は本来 active window に依存しないので、
# ここで mismatch するのは TMUX_PANE が stale だった等の異常系。即 abort して副作用を残さない
...
# Step 4 以降で source して CODEX_PANE / PARENT_WIN を復元する。
#
...
[ -n "$CODEX_PANE" ] && [ -n "$PARENT_WIN" ] \
  || { echo "[guard:state-missing] state file ($STATE_FILE) 不在または不完全 — Step 1 からやり直し" >&2; return 1 2>/dev/null || exit 1; }

# 1. pane がグローバルに存在するか（-a で全 session/window を走査、scope 暗黙依存を避ける）
tmux list-panes -a -F '#{pane_id}' | grep -qx "$CODEX_PANE" \
...
if ! echo "$last_line" | grep -qE '[❯›]'; then
  echo "[guard:not-ready] codexがプロンプト待ちではない。Step 2 の完了検知ループを再実行して待機" >&2
  # Step 2 を再実行してから本ガードに戻る
fi
```
...
rm -f /tmp/codex-prompt.txt "${XDG_STATE_HOME:-$HOME/.local/state}/codex-tmux/state.env"
```
