# `container` CLI 詳細リファレンス

apple/container の主要コマンド詳細（`create`/`start` や `builder status`/`delete`、各種 `--all`/`--force` 系フラグ等の網羅ではない。不明なコマンドは `container <cmd> --help` で確認する）。日常操作は SKILL.md のクイックリファレンスで足りる。
このファイルは特定セクションだけ読めばよい。

## 目次

- [動作要件](#動作要件) — macOS 15 での制限、対応アーキテクチャ
- [インストール・起動](#インストール起動) — install / upgrade / uninstall / 起動確認
- [CLI の全体構造](#cli-の全体構造) — サブコマンドツリー一覧
- [コンテナを動かす（container run）](#コンテナを動かすcontainer-run) — リソース・マウント・ポート・SSH・capability・init・ネスト仮想化
- [イメージのビルド（container build）](#イメージのビルドcontainer-build) — ビルダー VM のリソース調整
- [イメージ管理](#イメージ管理) — pull/push/save/load、レジストリ認証
- [コンテナ管理](#コンテナ管理) — logs/exec/cp/stats/export
- [ネットワーク（macOS 26+）](#ネットワークmacos-26) — 隔離ネットワーク、ローカル DNS ドメイン
- [ボリューム](#ボリューム) — 名前付き/匿名、ext4 ジャーナルモード
- [コンテナマシン（container machine）](#コンテナマシンcontainer-machinealias-m) — 永続 Linux 開発環境
- [システム](#システム) — カーネル管理、プロパティ
- [設定ファイル](#設定ファイル-configcontainerconfigtoml) — config.toml 全セクション
- [シェル補完](#シェル補完)
- [既知の制限](#既知の制限)
...
`m` は `container machine` の alias（この Mac では単体コマンドとして未設定のため `container m` の形で使う）。

```bash
container machine create alpine:latest --name dev
container machine set-default dev
container m run                                  # 対話シェル（既定マシン）
container m run -n dev whoami                    # ホストのユーザー名が返る
container m run -n dev pwd                       # /Users/<you>（Mac の $HOME がそのままマウントされている）
container m run -n dev -- cat /proc/cpuinfo
container m ls
container m inspect dev
container m stop dev
container m rm dev                               # 永続ストレージごと削除
```
...
container m set -n dev home-mount=ro             # ro / rw / none
container m set -n dev virtualization=true kernel=/path/to/vmlinux-kvm
container m set -n dev kernel=                   # カスタムカーネル解除
container m stop dev && container m run -n dev -- nproc
```
