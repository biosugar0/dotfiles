...
PR 説明でレビュアーの質問を先回りする:
- migration・feature flag・データ移行を含む PR は、backfill 要否（理由付き）／flag 切替手順と
  無効期間中のデータの扱い（取りこぼし・一斉処理の有無）／過去データへの影響／ロック影響 を明記する
- 新しいアーキテクチャパターン（outbox・CAS・lease 等）を導入する PR は、パターンが成立する
  ための不変条件を PR 説明に列挙する
...
- タスク開始・plan 前・繰り返し失敗・evaluator BLOCKED/NOT_EVALUABLE 時 → consult-memory（記憶参照, 限定タイミング）
- evaluator PASS 後の教訓記録 → distill-memory（schema 必須・昇格は人間承認・`ai-memory-prune` で衛生）
