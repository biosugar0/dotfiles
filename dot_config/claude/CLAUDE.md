...
- **頻度抑制（安い）**: 破損頻度そのものを下げるため、重い Bash はインライン展開せずスクリプト化／`bin/` ヘルパー経由（Golden Rule 既出）。tool 呼び出し直前の前置きテキストは短く。リトライに頼るより安い。
- **最終手段**: 同一破損が2連続すると stop-hook が `/model sonnet` 切替を促す（無限再破損防止）。shell 反復の重ループ・長時間タスクは自分から Sonnet に落とすか subagent 委譲でも良い。
